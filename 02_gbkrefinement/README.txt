...
The script gbkrefinement was developed to enrich the gene entries of your annotation (GenBank format) with EC numbers from the associated gene entries of 
the KEGG Database (Internet connection required!). If the found EC number in the associated KEGG entrie is not in the gene entrie of your annotation then the EC number is added. The script 
generates an Report file with the EC numbers which were added and the improved annotation in GenBank format.
...
improved_ecoli_k_12.gbk
	...
	This genome annotation of Escherichia coli str. K-12 substr. MG1655 is the output annotation from the 01_biopython_parser script. It is based on 
	the annotation which can be found under:
...
gbk_ref_improved_ecoli_k_12.gbk
	This is a sample refined GenBank annotation which is produced by running "gbkrefinement" with improved_ecoli_k_12.gbk as INPUT. The genes in your 
	annotation were enriched with EC numbers from the gene associated entries from the KEGG database. 
